...
class OccupancyClient {
  constructor() {
    ...
    this.Events = {
      [USAGE_DATA_TYPES.startData]: USAGE_DATA_TYPES.startData,
      [USAGE_DATA_TYPES.lineData]: USAGE_DATA_TYPES.lineData,
    };
      ...
      onData: (e) => {
        ...
        switch (e.type) {
          case USAGE_DATA_TYPES.startData:
            // 初始化表(pid信息)
            this._eventBus.emit(USAGE_DATA_TYPES.startData, e.data);
          ...
          case USAGE_DATA_TYPES.lineData:
            this._eventBus.emit(USAGE_DATA_TYPES.lineData, e.data);
            // 创建occupy记录
